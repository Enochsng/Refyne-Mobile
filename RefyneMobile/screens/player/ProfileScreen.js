  ...
  const handleChangeProfilePhoto = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        // Upload to backend → Supabase Storage, then persist URL in auth metadata (profiles.avatar_url syncs via trigger)
        try {
          ...
          // Cache URL locally so UI and getPlayerProfilePhoto work without waiting for session refresh
          await AsyncStorage.setItem(`profile_photo_${user.id}`, publicUrl);
          setProfilePhotoUri(publicUrl);
          Alert.alert('Success', 'Profile photo updated successfully!');
        } catch (uploadErr) {
          console.error('Avatar upload failed:', uploadErr);
          // Fallback: keep local URI only (previous behavior; won’t persist across devices)
