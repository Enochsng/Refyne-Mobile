-- Safe Database Setup Script for Refyne Mobile
-- This script handles existing tables and columns properly

-- Drop existing tables if they exist (in reverse order to handle dependencies)
DROP TABLE IF EXISTS messages CASCADE;
DROP TABLE IF EXISTS conversations CASCADE;
DROP TABLE IF EXISTS payment_transfers CASCADE;
DROP TABLE IF EXISTS coaching_sessions CASCADE;
DROP TABLE IF EXISTS coach_connect_accounts CASCADE;
...
CREATE INDEX idx_coach_connect_accounts_coach_id ON coach_connect_accounts(coach_id);
CREATE INDEX idx_coach_connect_accounts_stripe_account_id ON coach_connect_accounts(stripe_account_id);

CREATE INDEX idx_coaching_sessions_coach_id ON coaching_sessions(coach_id);
CREATE INDEX idx_coaching_sessions_payment_intent_id ON coaching_sessions(payment_intent_id);
CREATE INDEX idx_coaching_sessions_status ON coaching_sessions(status);

CREATE INDEX idx_payment_transfers_coach_id ON payment_transfers(coach_id);
CREATE INDEX idx_payment_transfers_payment_intent_id ON payment_transfers(payment_intent_id);
